...
const SparkLine = () => (
  <svg viewBox="0 0 80 28" className="w-full h-7" preserveAspectRatio="none">
    ...
    <path
      d="M0,22 L13,18 L27,10 L40,14 L53,6 L67,9 L80,4 L80,28 L0,28 Z"
      fill="url(#sg)"
    />
    <path
...
export default async function DashboardPage() {
  ...
  // Fetch locations to check if we should hide the promo banner
  ...
  const currentMonthTotal = currentMonthRides.reduce(
    (acc, r) => acc + Number(r.amount || 0),
    0,
  );
  const lastMonthTotal = lastMonthRides.reduce(
    (acc, r) => acc + Number(r.amount || 0),
    0,
  );
    ...
    <div className="p-6 lg:p-8 max-w-5xl mx-auto space-y-6 animate-fade-in-up">
      ...
      <div className="grid gap-4 sm:grid-cols-3 flex-col flex sm:grid">
        ...
        <Card className="bg-zinc-950 border-zinc-900 rounded-xl shadow-sm order-2">
          ...
          <CardContent className="px-5 pb-5">
            <div className="text-[28px] font-bold text-zinc-100 tracking-tight leading-none mt-1">
              {receipts.length}
            </div>
            ...
            <div className="flex gap-0.5 mt-4 h-1 rounded-full overflow-hidden bg-zinc-900">
              ...
              <div
                ...
                style={{ width: `${rapidoPct}%` }}
                title="Rapido"
              />
            </div>
            <div className="flex gap-4 mt-2">
              {[
                ["Uber", "bg-zinc-300", uberCount],
                ["Rapido", "bg-yellow-400", rapidoCount],
        ...
        <Card className="bg-zinc-950 border-zinc-900 rounded-xl shadow-sm order-4 sm:order-3">
          ...
          <CardContent className="px-5 pb-5">
            <div className="flex items-center gap-2 mt-1">
              <span
                className={`h-2 w-2 rounded-full shrink-0 ${isConnected ? "bg-emerald-500 animate-pulse-dot" : "bg-amber-500"}`}
              />
      ...
      <Card className="bg-zinc-950 border-zinc-900 rounded-xl shadow-sm">
        <CardHeader className="border-b border-zinc-900/80 pb-4 pt-5 px-5 flex flex-row items-center justify-between">
          <CardTitle className="text-[15px] font-bold text-zinc-100">
            Recent Rides
          </CardTitle>
          <Link
          ...
          {hasRides ? (
            <Table>
              <TableHeader>
                <TableRow className="hover:bg-transparent border-zinc-900/80">
                  <TableHead className="text-zinc-600 font-semibold text-[10px] uppercase tracking-widest pl-5">
                    Date
                  </TableHead>
                  <TableHead className="text-zinc-600 font-semibold text-[10px] uppercase tracking-widest">
                    Service
                  </TableHead>
                  <TableHead className="text-zinc-600 font-semibold text-[10px] uppercase tracking-widest">
                    Amount
                  </TableHead>
                  <TableHead className="text-zinc-600 font-semibold text-[10px] uppercase tracking-widest text-right pr-5">
                    Status
                  </TableHead>
                </TableRow>
              </TableHeader>
              <TableBody>
                {receipts.slice(0, 5).map((ride) => (
                  <TableRow
                    key={ride.id}
                    className="border-zinc-900/60 hover:bg-zinc-900/20 transition-colors cursor-default"
                  >
                    ...
                    <TableCell className="py-3">
                      <div className="flex items-center gap-2">
                        <span
                          className={`h-1.5 w-1.5 rounded-full shrink-0 ${serviceColor[ride.service] ?? "bg-zinc-500"}`}
                        />
                        <span className="text-zinc-200 text-[13px] font-medium capitalize">
                          {ride.service}
                        </span>
                      </div>
                    </TableCell>
            ...
            <div className="flex flex-col items-center justify-center py-16 px-6 text-center">
              ...
              <p className="text-xs text-zinc-600 max-w-[22rem] leading-relaxed mb-5">
                Connect your Gmail account and Claimo will automatically find
                your Uber and Rapido ride receipts.
              </p>
